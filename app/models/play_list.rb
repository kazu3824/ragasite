class PlayList < ApplicationRecord
  ...
  def update_relations(track_ids)
    ...
    # self.tracks.ids&.map(&:to_s)はtrack_idsでもtracks.idsでも同じ挙動。
    old_track_ids = self.tracks.ids&.map(&:to_s)
    ...
    # 中間テーブル(LineItem)にある、古いtrack_idsをidsの引き算をしてwhere検索して該当するレコードすべてを削除する
